...
export default function RoomCard({ room, className }: RoomCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full h-64 sm:h-72 overflow-hidden bg-brown-light shrink-0">
        <Image
          src={primaryImage}
          alt={altText}
          fill
        ...
        {room.badge && (
          <div className="absolute top-4 left-4 z-10">
            <span className="px-3 py-1 bg-black/75 backdrop-blur-md text-gold text-xs font-sans font-semibold rounded-full border border-gold/40 shadow-sm">
              {room.badge}
            </span>
          </div>
        )}
        ...
        {/* Gold Pricing Badge */}
        <div className="absolute bottom-4 right-4 z-10 bg-gold text-black px-4 py-2 rounded-xl shadow-gold text-right backdrop-blur-sm">
          ...
          {features.map((feature) => {
            ...
            return (
              <div key={feature} className="flex items-center gap-2">
                <div className="w-7 h-7 rounded-lg bg-gold/10 border border-gold/25 flex items-center justify-center shrink-0">
